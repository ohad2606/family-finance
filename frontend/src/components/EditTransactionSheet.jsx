...
export default function EditTransactionSheet({ tx, onClose }) {
  ...
  const mutation = useMutation({
    mutationFn: data => updateTransaction(tx.id, data),
    onSuccess: () => {
  ...
  return (
    <BottomSheet onClose={onClose}>
        <h2 style={styles.title}>עריכת תנועה</h2>
        ...
        <form onSubmit={submit} style={styles.form}>
          ...
          <button style={{ ...styles.submitBtn, background: form.kind === 'income' ? C.income : C.expense }}
            ...
            {mutation.isPending ? '...' : 'שמור שינויים'}
          </button>
        </form>
    </BottomSheet>
  )
